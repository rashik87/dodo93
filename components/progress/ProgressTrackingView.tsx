...
const ProgressTrackingView: React.FC<ProgressTrackingViewProps> = ({
  ...
  const handleCalculateBodyFat = () => {
    setFormError(null);
    if (!userDataForCalc || !userDataForCalc.height || !userDataForCalc.gender || !userDataForCalc.age) {
    ...
    const analysisResult = analyzeBodyComposition(userDataForCalc.gender, userDataForCalc.age, userDataForCalc.height, weightKgNum, neckCm, waistCm, hipCmNum);
  ...
  const canCalculateBfp = !!(userDataForCalc?.height && userDataForCalc.gender && userDataForCalc.age);
